...
#'               alleles.
#'
# this is actually a n.check x n.loci array with each non-zero element the
# number of potentially non-null matches there.  do i need two modes?  i think
# no.
#' @param mode   \code{"locus"} or \code{"column"}.  Determines how each locus
...
#'
# an array of coordinates n.check x n.loci, non-zero elements potential nulls
# in genotypes
...
checkNullAlleles <- function(ref, check, mode = c("locus", "column"),
    ...
    null.locations <-
        array(0, dim = c(n.check, n.loci), dimnames = list(check.names, loci))
        ...
        for (k in 1:n.ref) {
            ...
            for (locus in 1:n.loci) {
                ...
                if (all(sort(c(m1, m2)) == sort(c(p1, p2)))) {
                    ...
                    null.locations[j, locus] <- null.locations[j, locus] + 1
                } else break
            }
            if (match.pmatch == n.loci) {
                ...
                name.potential.null.loci[j, k] <- paste(collapse=",",
                                                       potential.null.loci)
                if (! quiet && length(potential.null.loci) > 0) {
                    # print it out
                    if (! check.printed) {
                        check.printed <- TRUE
                        printGenotype(check, j, label = "check", sep = "\t",
                                      callout.locus = potential.null.loci)
                    }
                    printGenotype(ref, k, label = "ref", sep = "\t",
                                  callout.locus = potential.null.loci)
                }
            }
        }
